...
> **Lưu ý**: Các ví dụ ở đây sử dụng tên thiết bị `/dev/sdb` cùng các phân vùng lưu trữ của nó. Tên thiết bị trên hệ thống của bạn có
> thể sẽ khác. Hãy sử dụng các lệnh `lsblk`, `blkid` hoặc `cat /proc/partitions` để xác định các thiết bị trên hệ thống của bạn.
