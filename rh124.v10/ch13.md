...
Bạn có thể sử dụng lệnh `flatpak` hoặc công cụ đồ họa GNOME Software để quản lý các gói. Với bất kỳ công cụ nào trong số này, bạn đều có thể cấu hình nhiều kho lưu trữ từ xa và đơn giản hóa việc quản lý Flatpak thông qua các giao diện thống nhất.
...
Trong cửa sổ bật lên Software Repositories, các ứng dụng Flatpak được liệt kê tại mục Apps (Flatpak). Từ cửa sổ này, bạn có thể bật hoặc tắt các kho lưu trữ từ xa đã được cấu hình.
...
Các ứng dụng và runtime Flatpak sử dụng các định danh duy nhất tuân theo định dạng `domain.organization.App`. Định danh gồm ba phần này cho phép công ty, nhà phát triển hoặc tổ chức phát hành nhiều ứng dụng có chung tiền tố, chẳng hạn như `com.company`. Phần cuối
